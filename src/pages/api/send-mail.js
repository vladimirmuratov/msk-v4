...
export default function handler(req, res) {
    ...
    const message1 = {
        ...
        html: ` <p>Имя: ${req.body.person}</p>
                ...
                <p>Сообщение пришло с сайта: delo-zhizni.mskdoctor.ru</p>`,
    }
    ...
    if(req.body.email){
        const message2 = {
            from: process.env.ADDRESS_FROM,
            to: [req.body.email],
            subject: `Вы оставили заявку на звонок на сайте delo-zhizni.mskdoctor.ru в ${new Date().toLocaleString('ru-RU', {timeZone: 'Europe/Moscow'})}`,
            html: ` <p>Имя: ${req.body.person}</p>
                    <p>Дата рождения: ${req.body.dateBirth ?? 'Нет'}</p>
                    <p>Телефон: ${req.body.phone}</p>
                    <p>Email: ${req.body.email ?? 'Нет'}</p>
